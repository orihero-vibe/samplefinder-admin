...
interface CheckUsernamePhoneAvailabilityRequest {
  username?: string;
  phoneNumber?: string;
}
...
async function submitTriviaAnswer(
  ...
  if (isCorrect) {
    ...
    const currentTriviasWon =
      Number((userDoc as Record<string, unknown>).triviasWon) || 0;
    ...
    log(
      `Awarded ${pointsAwarded} points to user ${userId}; triviasWon ${currentTriviasWon} -> ${currentTriviasWon + 1}`
    );
  }
...
async function dismissTrivia(
  ...
  triviaId: string,
  log: (message: string) => void
): Promise<{ correctAnswerIndex: number }> {
  try {
    await databases.getDocument(DATABASE_ID, USER_PROFILES_TABLE_ID, userId);
  } catch {
    throw { code: 404, message: 'User not found' };
...
async function checkUsernamePhoneAvailability(
  _users: Users,
  databases: Databases,
  data: CheckUsernamePhoneAvailabilityRequest,
  log: (message: string) => void
): Promise<{ usernameExists?: boolean; phoneExists?: boolean }> {
  const result: { usernameExists?: boolean; phoneExists?: boolean } = {};
  ...
  if (rawUsername) {
    ...
    const usernameResult = await databases.listDocuments(
      DATABASE_ID,
      USER_PROFILES_TABLE_ID,
      [Query.limit(1000)]
    );
  ...
  log(
    `[check-username-phone-availability] usernameChecked=${Boolean(rawUsername)} phoneChecked=${Boolean(rawPhone)}`
  );
  return result;
}
...
export default async function handler({
  ...
  try {
    ...
    if (req.path === '/dismiss-trivia' && req.method === 'POST') {
      ...
      try {
        const dismissResult = await dismissTrivia(
          databases,
          body.userId,
          body.triviaId,
          log
    ...
    if (
      req.path === '/check-username-phone-availability' &&
      req.method === 'POST'
    ) {
      log('Processing check-username-phone-availability request');

      const body = (req.body || {}) as CheckUsernamePhoneAvailabilityRequest;

      try {
        const result = await checkUsernamePhoneAvailability(
          users,
          databases,
          body,
          log
